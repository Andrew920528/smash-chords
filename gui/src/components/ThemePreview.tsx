    ...
    <div className="p-8 space-y-8 bg-background text-foreground min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {themeColors.map((color) => (
          <div key={color.name} className="space-y-3 p-4 border rounded-lg border-border">
            <h3 className="font-medium">{color.name}</h3>
            
            {/* Full Opacity */}
            <div className={`h-16 w-full rounded ${color.variable} ${color.text} flex items-center justify-center text-xs font-bold`}>
              Default (100%)
            </div>
            
            {/* Testing Tailwind Opacity Modifier */}
            <div className="flex gap-2">
              <div className={`h-10 flex-1 rounded ${color.variable}/50 flex items-center justify-center text-[10px]`}>
                50% Alpha
              </div>
              <div className={`h-10 flex-1 rounded ${color.variable}/20 flex items-center justify-center text-[10px]`}>
                20% Alpha
              </div>
            </div>
          </div>
      ...
      <div className="space-y-4 pt-8 border-t border-border">
        ...
        <p className="text-muted-foreground italic">
          Note: If boxes above are transparent or white, your SCSS variables are not reaching the global scope.
        </p>
      </div>
    </div>
  );
